...
public interface DpRepository extends JpaRepository<Disease_Pest, Long>{
	...
	@Query("SELECT dp FROM Disease_Pest dp WHERE " +
	           "(:crop IS NULL OR dp.crop = :crop) AND " +
	           "(:season IS NULL OR dp.season = :season) AND " +
	           "(:site IS NULL OR dp.site = :site) AND " +
	           "(:argu IS NULL OR dp.argu = :argu)")
	    List<Disease_Pest> findByCropOrSeasonAndSiteAndArgu(
	            @Param("crop") Crop crop,
	            @Param("season") String season,
	            @Param("site") String site,
	            @Param("argu") String argu);
}